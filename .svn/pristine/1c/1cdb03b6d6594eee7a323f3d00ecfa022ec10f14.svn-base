...
$(function() {
	...
	compModifyBtn.on("click", function(){
		...
		if(updFlag != false) {
			...
			$("input[name=cusPw]").removeAttr("readonly");
			$("input[name=cusPw2]").removeAttr("readonly");
			$("input[name=cusEmp]").removeAttr("readonly");
// 			$("img[id=nowImg]").removeAttr("src");
	...
	//둘다 false, true로 출력됨.
// 	$("#vacWgPay").on("change",function(){
// 		console.log("vacWgPay : " + $(this).is(":checked"));
// 	});
// 	$("#vacCert").on("change",function(){
// 		console.log("vacWgPay : " + $(this).is(":checked"));
// 	});
// 	$("#testPay").on("click",function(){	//vacWgPay가 checked일때에 둘다 false쪽으로 체크해제시켜줌
// 		if($("#vacWgPay").is(":checked") == true){
// 			$("#vacWgPay").prop("checked", false);
// 			$("#vacCert").prop("checked", false);
// 		}
// 	});
	
// 	console.log(JSON.stringify(data) + "@@@@@@@@" + vacName);
...
// 		tmpList.push("empNo",empNo);
// 		tmpList.push("empRole",changeValue);
