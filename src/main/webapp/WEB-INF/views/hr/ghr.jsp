...
=======
							...
							<form action="/hr/ghrExcel" method="get">
					            <button type="submit" class="btn btn-light" style="width: 45px; height: 38px;">
									<i class="bx bx-download" data-feather="download" style="width: 20px; height: 20px; margin-bottom: 3px"></i>
								</button>
					        </form>
						</div>
								...
								<tbody>
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${ghrList }" var="hr">
												<tr onclick="location.href='/mypage/other?empNo=${hr.empNo }'">
													<td><img src="${pageContext.request.contextPath }${hr.empProfile}"
													...
													<td>
														<div class="d-flex gap-2">
															<c:if test="${hr.empPos eq '직책 없음' }">
																<div class="badge badge-soft-secondary">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq '파트장' }">
																<div class="badge badge-soft-warning">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq '팀장' }">
																<div class="badge badge-soft-success">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq '실장' }">
																<div class="badge badge-soft-purple">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq '본부장' }">
																<div class="badge badge-soft-info">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq 'CFO' }">
																<div class="badge badge-soft-primary">${hr.empPos }</div>
															</c:if>
															<c:if test="${hr.empPos eq 'CEO' }">
																<div class="badge badge-soft-danger">${hr.empPos }</div>
															</c:if>
														</div>
													</td>
												</tr>
  ...
  selectedNodeIDs.forEach(function (nodeID) {
	  ...
	  $.ajax({
    	  ...
    	  for(let i = 0; i<response.length; i++) {
			  ...
			  var row = $('<tr>'); 
			  var cell1 = $('<td>').html('<img src="' + rowData.empProfile + '" alt="" class="avatar-sm rounded-circle me-2"><a href="/mypage/other?empNo='+ rowData.empNo + '" class="text-body fw-medium">' + rowData.empName + '</a>');
			  var cell2 = $('<td>').text(rowData.empDept);
			  ...
			  function badge(empPos) {
			      switch (empPos) {
			          case '직책 없음':
			              return '<div class="badge badge-soft-secondary">' + empPos + '</div>';
			          case '파트장':
			              return '<div class="badge badge-soft-warning">' + empPos + '</div>';
			          case '팀장':
			              return '<div class "badge badge-soft-success">' + empPos + '</div>';
			          case '실장':
			              return '<div class="badge badge-soft-purple">' + empPos + '</div>';
			          case '본부장':
			              return '<div class="badge badge-soft-info">' + empPos + '</div>';
			          case 'CFO':
			              return '<div class="badge badge-soft-primary">' + empPos + '</div>';
			          case 'CEO':
			              return '<div class="badge badge-soft-danger">' + empPos + '</div>';
			          default:
			              return empPos;
